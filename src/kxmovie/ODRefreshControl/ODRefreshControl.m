...
+ (id)newWithScroll:(UIScrollView *)scrollView
            actType:(ODRefreshActivityType)type
            actView:(UIView *)activity
{
    switch (type) {
        case ODRefreshActivityTypeSystem:
        {
            return [[ODRefreshControl alloc] initInScrollView:scrollView activityIndicatorView:nil];
        }
            break;
            case ODRefreshActivityTypeOther:
        {
            return [[ODRefreshControl alloc] initInScrollView:scrollView activityIndicatorView:activity];
        }
            break;
        default:
        {
            V_animateIndicator *v = [[V_animateIndicator alloc] initWithFrame:CGRectMake(0.f, 0.f, 20.f, 20.f)];
            return [[ODRefreshControl alloc] initInScrollView:scrollView activityIndicatorView:v];
        }
            break;
    }
}
