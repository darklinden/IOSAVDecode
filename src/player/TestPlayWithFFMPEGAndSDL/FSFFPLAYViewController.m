...
- (void)seekWithTime:(int)time {
    ...
    int64_t pos = 0;
    if(videoState) {
        if (!videoState->abort_request) return;
        pos = get_master_clock(videoState);
        pos += incr;
        //        stream_seek(videoState, (int64_t)(pos * AV_TIME_BASE), incr);
        stream_seek(videoState, (int64_t)(pos * AV_TIME_BASE), time, 0);
    }
}
...
- (IBAction)playAction:(id)sender {
    //    [self startPlayWithURLString:[[NSBundle mainBundle] pathForResource:@"1" ofType:@"mov"]];
    //    [self startPlayWithURLString:[[NSBundle mainBundle] pathForResource:@"2" ofType:@"avi"]];
    [self startPlayWithURLString:@"http://eric.cast.ro/stream2.flv"];
//    NSString *path = [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) lastObject] stringByAppendingPathComponent:@"sample_h264_300kbit.mp4"];
//    [self startPlayWithURLString:path];
}
...
- (IBAction)seekPlayAction:(id)sender {
    [self seekWithTime:100];
}
